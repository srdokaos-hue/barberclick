...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title:`Agendar — ${shop.name}`, description:`Agende seu horário na ${shop.name}` }
}
...
export default async function PublicBookingPage({ params }: Props) {
  const barbershop = await prisma.barbershop.findUnique({
    ...
    include: {
      barbers:  { where:{ active:true, role:"BARBER" }, select:{ id:true, name:true, avatarUrl:true, commissionPct:true }, orderBy:{ name:"asc" } },
      services: { where:{ active:true }, orderBy:{ price:"asc" } },
      scheduleConfig: true,
      products: { where:{ active:true, stockQty:{ gt:0 } }, take:1, orderBy:{ salePrice:"asc" } },
    },
  })
  ...
  return (
    <BookingPageClient
      barbershop={{ id:barbershop.id, name:barbershop.name, slug:barbershop.slug, whatsapp:barbershop.whatsapp??undefined }}
      services={barbershop.services.map(s=>({
        id:s.id, name:s.name, price:Number(s.price),
        durationMin:s.durationMin, description:s.description??"",
      }))}
      barbers={barbers.map(b=>({
        id:b.id, name:b.name,
        avatarUrl:b.avatarUrl??undefined,
        commissionPct:Number(b.commissionPct),
      }))}
