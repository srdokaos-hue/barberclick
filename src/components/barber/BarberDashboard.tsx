...
import { Check, User, ChevronLeft, ChevronRight, TrendingUp, Scissors, Calendar } from "lucide-react"
    ...
    <div style={{padding:"24px",maxWidth:720,margin:"0 auto"}}>
      ...
      <div style={{marginBottom:20}}>
        ...
        <div style={{fontSize:20,fontWeight:600,color:"var(--color-text-primary)"}}>
          Olá, {s?.name?.split(" ")[0]||"barbeiro"}! ✂️
        </div>
        <div style={{fontSize:13,color:"var(--color-text-tertiary)",marginTop:2}}>
          Comissão: <strong>{s?.commissionPct||0}%</strong> sobre serviços realizados
        </div>
      </div>
        ...
        <div style={{padding:"14px 16px",borderBottom:"1px solid var(--color-border-tertiary)",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
          <div style={{display:"flex",alignItems:"center",gap:6,flex:1}}>
            ...
            <span style={{fontSize:14,fontWeight:600,color:"var(--color-text-primary)"}}>
              {date===today?"Hoje":new Date(date+"T12:00:00").toLocaleDateString("pt-BR",{weekday:"long",day:"numeric",month:"short"})}
            </span>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <button onClick={prevDay} style={{width:30,height:30,borderRadius:7,border:"1px solid var(--color-border-tertiary)",background:"var(--color-background-secondary)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",padding:0}}>
              <ChevronLeft size={14}/>
            </button>
            ...
            <button onClick={nextDay} style={{width:30,height:30,borderRadius:7,border:"1px solid var(--color-border-tertiary)",background:"var(--color-background-secondary)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",padding:0}}>
