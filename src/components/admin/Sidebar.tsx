...
export default function Sidebar() {
  ...
  // Fecha o menu quando muda de página no mobile
  ...
  const sidebarContent = (
    <div style={{ display:"flex", flexDirection:"column", height:"100%", background:"var(--sidebar-bg)", color:"var(--sidebar-fg)" }}>
      ...
      <div style={{ padding:"18px 20px 16px", display:"flex", alignItems:"center", gap:10, borderBottom:"1px solid rgba(255,255,255,.08)" }}>
        ...
        <span style={{ fontSize:15,fontWeight:600,flex:1 }}>BarberClick</span>
        {mobile && (
          <button onClick={()=>setOpen(false)} style={{ background:"transparent",border:"none",color:"rgba(255,255,255,.6)",cursor:"pointer",padding:4,display:"flex",alignItems:"center",justifyContent:"center" }}>
            <X size={20}/>
          </button>
        )}
      </div>
  ...
  if (mobile) return (
    <>
      ...
      <div style={{
        ...
        background:"var(--sidebar-bg)", borderBottom:"1px solid rgba(255,255,255,.08)",
        display:"flex", alignItems:"center", padding:"0 16px", gap:12,
      }}>
        <button onClick={()=>setOpen(true)} style={{ background:"transparent",border:"none",color:"white",cursor:"pointer",padding:4,display:"flex",alignItems:"center" }}>
          <Menu size={22}/>
        </button>
        <div style={{ display:"flex",alignItems:"center",gap:8 }}>
          <div style={{ width:26,height:26,background:"white",borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center" }}>
            <Scissors size={14} color="#111"/>
          </div>
          <span style={{ fontSize:15,fontWeight:600,color:"white" }}>BarberClick</span>
        </div>
        <div style={{ marginLeft:"auto",fontSize:12,color:"rgba(255,255,255,.5)",maxWidth:160,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>
      ...
      {/* Overlay escuro */}
      {open && (
        <div onClick={()=>setOpen(false)} style={{ position:"fixed",inset:0,background:"rgba(0,0,0,.6)",zIndex:200 }}/>
      )}
      ...
      <div style={{
        ...
        boxShadow:open?"4px 0 20px rgba(0,0,0,.4)":"none",
      }}>
        {sidebarContent}
      </div>

      {/* Espaço para o conteúdo não ficar atrás da topbar */}
      <div style={{ height:56 }}/>
    </>
  )
